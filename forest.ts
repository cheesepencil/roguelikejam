...
export class Forest {
  ...
  constructor(width: number, height: number, seed: string[]) {
    ...
    while (passes < 4) {
      ...
      for (let i = 0; i < starters.length; i++) {
        ...
        let nextNode =
          candidateNodes[this.randomizer.between(0, candidateNodes.length - 1)];
      ...
      for (let j = 0; j < this.height; j++) {
        ...
        if (node.connections.length === 0) {
          ...
          if (visitedNeighbors.length > 0) {
            let neighbor =
              visitedNeighbors[
                this.randomizer.between(0, visitedNeighbors.length - 1)
              ];
  ...
  getNeighbors(node: ForestNode): ForestNode[] {
    ...
    for (let i = 0; i < modifiers.length; i++) {
      let horizontalNeighbor = this.getNode(node.x + modifiers[i], node.y);
